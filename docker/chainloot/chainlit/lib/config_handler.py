...
prompt_catalog = {
    "3PO": "You are a 3-P-O, a helpful AI assistant. Your responses are concise and brief. No more than 2 sentences per message.",
    "Yoda": "You are Yoda, wise Jedi Master. No more than 2 sentences per message.",
    "Stark": "You are a helpful but snarky AI assistant. Your name is Tony. No more than 2 sentences per message."
}

chat_backend_base_url = config.get("chat_backend_base_url", "").rstrip('/v1')
chatterbox_url = config.get("tts_base_url", "")
api_key = os.getenv("CHAT_API_KEY", config.get("api_key"))

# --- API Client Initialization ---
# Ensure URLs are not empty before creating clients
if not chat_backend_base_url:
    logger.error("chat_backend_base_url not found in config.json. Exiting.")
    sys.exit(1)
if not chatterbox_url:
    logger.error("chatterbox_url not found in config.json. Exiting.")
    sys.exit(1)

client = AsyncOpenAI(base_url=f"{chat_backend_base_url}/v1", api_key=api_key)
...
def get_client():
    ...
    if provider == "ollama":
        ...
        base_url = f"{chat_backend_base_url}/v1"
        api_key = "docker-model-runner"
    else:
        # Default to Docker Model Runner
        base_url = f"{chat_backend_base_url}/v1"
...
def fetch_available_models(provider=None):
    ...
    try:
        if provider == "ollama":
            ...
            # Docker Model Runner uses OpenAI-compatible /models endpoint
            dmr_url = f"{chat_backend_base_url}/v1/models"
            response = requests.get(dmr_url, timeout=10)
            response.raise_for_status()
            models_data = response.json()
