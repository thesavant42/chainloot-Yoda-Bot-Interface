#!/usr/bin/env python3
"""
GPU Monitor Database Load Test - 7 Days

This script generates synthetic GPU metrics data for 7 days and populates the SQLite
database to test the 7-day history feature.

Usage:
    python3 db_load_7d_test.py [--db-path PATH] [--interval SECONDS]
...
def parse_args():
    parser = argparse.ArgumentParser(description='GPU Monitor Database Load Test - 7 Days')
...
def generate_metrics(start_time, end_time, interval):
    ...
    for day_offset in range(8):  # 7 days + 1 for partial days
...
def create_history_json(conn, output_path, history_days=7):
...
def main():
    ...
    # Calculate start and end times for 7 days
    ...
    try:
        ...
        create_history_json(conn, history_path, history_days=7)
        ...
        print("\nNow you can run the monitor_gpu.sh script with the 7-day history setting.")
    finally:
        conn.close()
