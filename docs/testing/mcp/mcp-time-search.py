# mcp-time-search.py
# demo chainlit 2.8.3 app to use 
# mcp servers. This example shows how
# to use the time server and Brave Search
# Expects BRAVE_API_KEY in .env
...
async def on_mcp_connect(connection, session: ClientSession):
    ...
    try:
        ...
        print(f"Discovered tools for {connection.name}: {[t.name for t in tool_list_result.tools]}")
...
async def handle_time_query(message: cl.Message):
    ...
    if not session_name or not tool_name:
        await cl.Message(content="A 'time' tool could not be found among the connected MCP servers.").send()
        return

    mcp_connection = cl.context.session.mcp_sessions.get(session_name)
    if not mcp_connection:
        await cl.Message(content=f"Time server session '{session_name}' is not connected.").send()
        return
    ...
    async with cl.Step(type="tool", name=tool_name) as step:
        ...
        try:
            ...
            if parsed_result and 'datetime' in parsed_result:
                ...
                error_message = parsed_result.get('error') if isinstance(parsed_result, dict) else "unexpected or empty response"
                location_for_error = display_name or "the specified location"
                await cl.Message(content=f"Sorry, the time tool returned an error for **{location_for_error}**: `{error_message}`").send()

        except Exception as e:
            step.output = {"error": str(e)}
            await cl.Message(content=f"A critical error occurred with the time tool: {e}").send()
...
async def handle_search_query(message: cl.Message):
    ...
    """
    if not os.environ.get("BRAVE_API_KEY"):
        await cl.Message(content="Cannot perform search. The Brave API Key is not configured.").send()
        return

    session_name, tool_name = find_mcp_tool("search")
    if not session_name or not tool_name:
        await cl.Message(content="A 'search' tool could not be found among the connected MCP servers.").send()
        return

    mcp_connection = cl.context.session.mcp_sessions.get(session_name)
    if not mcp_connection:
        await cl.Message(content=f"Search server session '{session_name}' is not connected.").send()
        return
    ...
    if not query:
        await cl.Message(content="I'm not sure what you want me to search for. Please be more specific.").send()
        return
    ...
    async with cl.Step(type="tool", name=tool_name) as step:
        ...
        try:
            ...
            # The Brave Search MCP returns plain text, not JSON.
            # We'll check for the text content directly and display it.
            if result and result.content and len(result.content) > 0 and hasattr(result.content[0], 'text') and result.content[0].text:
                ...
                await cl.Message(content=f"Sorry, the search tool returned an error for query **'{query}'**: `{error_message}`").send()

        except Exception as e:
            step.output = {"error": str(e)}
            await cl.Message(content=f"A critical error occurred with the search tool: {e}").send()
