...
class MCPToolProcessor:
    ...
    async def should_use_tools(self, message: str) -> bool:
        ...
        # Check for MQTT-related queries
        if self._is_mqtt_query(message_lower):
            return True
    ...
    def _is_mqtt_query(self, message: str) -> bool:
        """Check if message is asking for MQTT operations"""
        mqtt_indicators = [
            "mqtt",
            "publish",
            "subscribe",
        ...
        return any(indicator in message for indicator in mqtt_indicators)
    ...
    async def process_with_tools(self, message: str) -> Optional[str]:
        ...
        # Try MQTT query
        if self._is_mqtt_query(message_lower):
            result = await self._handle_mqtt_query(message)
            if result:
                return result
    ...
    async def _handle_mqtt_query(self, message: str) -> Optional[str]:
        """Handle MQTT-related queries"""
        mqtt_manager = get_active_mcp_manager()
        
        # Try to find MQTT tools by name or capability
        publish_tool = None
        receive_tool = None
        
        # Check if we're using dynamic or legacy manager
        if hasattr(mqtt_manager, 'tools'):  # Dynamic manager
            # Look for specific tool names in the tools dictionary
            for tool_name in mqtt_manager.tools:
                if "publish" in tool_name.lower():
                    publish_tool = tool_name
                elif "receive" in tool_name.lower():
                    receive_tool = tool_name
        elif hasattr(mqtt_manager, 'tool_cache'):  # Legacy manager
            # Look for specific tool names in the tool cache
            for tool_name in mqtt_manager.tool_cache:
                if "publish" in tool_name.lower():
                    publish_tool = tool_name
                elif "receive" in tool_name.lower():
                    receive_tool = tool_name
        
        # If no tools found by name, try capability lookup
        if not publish_tool and not receive_tool:
            publish_tool = mqtt_manager.find_tool_by_capability("publish")
            receive_tool = mqtt_manager.find_tool_by_capability("receive")
        
        if not publish_tool and not receive_tool:
            return "Sorry, MQTT tools are not available right now."
        
        try:
            message_lower = message.lower()
            
            # Determine if this is a publish or receive operation
            if any(word in message_lower for word in ["publish", "send", "send to", "publish to"]):
                if not publish_tool:
                    return "Sorry, MQTT publish functionality is not available right now."
                
                # Extract topic and message from user input
                topic, mqtt_message = self._extract_mqtt_publish_data(message)
                
                if not topic:
                    return "I need a topic to publish to. Please specify a topic like 'sensors/temperature' or 'home/livingroom/lights'."
                
                if not mqtt_message:
                    return "I need a message to publish. Please specify what data to send."
                
                logger.info(f"Publishing MQTT message to topic '{topic}': {mqtt_message}")
                ...
                result = await mqtt_manager.call_tool(publish_tool, tool_params)
                
                if (result and result.content and len(result.content) > 0 
                    and hasattr(result.content[0], "text")):
                    
                    response_text = result.content[0].text
                    if "successfully" in response_text.lower() or "published" in response_text.lower():
                        return f"Successfully published message to topic '{topic}': {mqtt_message}"
                    else:
                        return f"MQTT publish result: {response_text}"
                
                return f"Message published to MQTT topic '{topic}'."
                
            elif any(word in message_lower for word in ["receive", "listen", "subscribe", "get message"]):
                if not receive_tool:
                    return "Sorry, MQTT receive functionality is not available right now."
                
                # Check for "any topic" or "any message" requests
                if any(phrase in message_lower for phrase in ["any topic", "any message", "first message", "next message", "what's happening"]):
                    logger.info("Listening for first message from any MQTT topic")
                    
                # Use # wildcard to subscribe to all topics
                tool_params = {
                    "topic": "#",
                    "timeout": 10  # 10 second timeout for any message
                }
                
                result = await mqtt_manager.call_tool(receive_tool, tool_params)
                
                if (result and result.content and len(result.content) > 0 
                    and hasattr(result.content[0], "text")):
                    
                    response_text = result.content[0].text
                    if "no message" in response_text.lower() or "timeout" in response_text.lower():
                        return "No MQTT messages received from any topic within 10 seconds."
                    else:
                        return f"First MQTT message received: {response_text}"
                
                return "Listened for messages on all topics but nothing was received."                # Extract topic for subscription
                topic = self._extract_mqtt_topic(message)
                
                if not topic:
                    return "I need a topic to listen to. Please specify a topic like 'sensors/temperature' or 'home/+/status', or say 'listen for any message' to monitor all topics."
                
                logger.info(f"Receiving MQTT message from topic '{topic}'")
                ...
                if (result and result.content and len(result.content) > 0 
                    ...
                    response_text = result.content[0].text
                    if "no message" in response_text.lower() or "timeout" in response_text.lower():
                        return f"No messages received from topic '{topic}' within the timeout period."
                    else:
                        return f"Received from MQTT topic '{topic}': {response_text}"
                
                return f"Listened to MQTT topic '{topic}' but no messages were received."
            
            else:
                # General MQTT query - provide help
                return ("I can help you monitor the Chainloot system via MQTT! You can:\n"
                       "• Listen for any message: 'What's happening on MQTT?' or 'Listen for any message'\n"
                       "• Check resource usage: 'Get memory usage from /chainloot/system/tts_webui/resources'\n" 
                       "• Monitor service status: 'Listen for updates on /chainloot/system/postgres/services'\n"
                       "• Check persona emotions: 'Subscribe to /chainloot/persona/yoda/emotions'\n"
                       "• Get system timestamps: 'Receive from /chainloot/system/+/timestamp'\n"
                       "• Monitor all system data: 'Subscribe to /chainloot/system/+'\n"
                       "• Check character status: 'Listen for persona status updates'\n"
                       "What system component or persona data would you like to monitor?")
            
        except Exception as e:
            logger.error(f"Error in MQTT query: {e}")
            return f"Sorry, there was an error with the MQTT operation: {str(e)}"
    ...
    def _extract_mqtt_publish_data(self, message: str) -> Tuple[Optional[str], Optional[str]]:
        """Extract topic and message data from publish request"""
        import re
        import json
        
        # Look for patterns like:
        # "publish temperature 23.5 to topic sensors/temp"
        # "send {\"temp\": 23.5} to sensors/temperature"
        # "publish to home/status message online"
        
        # Pattern 1: "publish [data] to [topic]"
        pattern1 = r'publish\s+(.+?)\s+to\s+(?:topic\s+)?([^\s]+)'
        ...
        # Pattern 2: "send [data] to [topic]"  
        pattern2 = r'send\s+(.+?)\s+to\s+(?:topic\s+)?([^\s]+)'
        ...
        # Pattern 3: "publish to [topic] message [data]"
        pattern3 = r'publish\s+to\s+(?:topic\s+)?([^\s]+)\s+(?:message\s+)?(.+)'
        ...
        # Pattern 4: Look for JSON data and topic separately
        json_pattern = r'\{[^}]+\}'
        json_match = re.search(json_pattern, message)
        
        topic_pattern = r'(?:topic|to)\s+([a-zA-Z0-9/_-]+)'
        topic_match = re.search(topic_pattern, message, re.IGNORECASE)
        
        if json_match and topic_match:
            data = json_match.group(0)
            topic = topic_match.group(1)
            return topic, data
        
        # Fallback: look for any topic-like string and use rest as message
        topic_fallback = r'(?:sensors|home|device|iot)/[a-zA-Z0-9/_-]+'
        topic_match = re.search(topic_fallback, message, re.IGNORECASE)
        if topic_match:
            topic = topic_match.group(0)
            # Use the part of message that's not the topic as data
            data = message.replace(topic, "").strip()
            # Clean up common words
            for word in ["publish", "send", "to", "topic", "message"]:
                data = re.sub(rf'\b{word}\b', '', data, flags=re.IGNORECASE).strip()
            if data:
                return topic, data
        
        return None, None
    
    def _extract_mqtt_topic(self, message: str) -> Optional[str]:
        """Extract MQTT topic from receive/subscribe request"""
        import re
        
        # Look for patterns like:
        # "listen on topic sensors/temp"
        # "subscribe to home/+/status"
        # "receive from sensors/temperature"
        
        # Pattern 1: "listen on/to [topic]"
        pattern1 = r'listen\s+(?:on|to)\s+(?:topic\s+)?([a-zA-Z0-9/_+-]+)'
        match = re.search(pattern1, message, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        
        # Pattern 2: "subscribe to [topic]"
        pattern2 = r'subscribe\s+to\s+(?:topic\s+)?([a-zA-Z0-9/_+-]+)'
        match = re.search(pattern2, message, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        
        # Pattern 3: "receive from [topic]"
        pattern3 = r'receive\s+from\s+(?:topic\s+)?([a-zA-Z0-9/_+-]+)'
        match = re.search(pattern3, message, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        
        # Pattern 4: "topic [topic]" or just look for topic-like strings
        pattern4 = r'topic\s+([a-zA-Z0-9/_+-]+)'
        match = re.search(pattern4, message, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        
        # Fallback: look for any topic-like string
        topic_pattern = r'(?:sensors|home|device|iot)/[a-zA-Z0-9/_+-]+'
